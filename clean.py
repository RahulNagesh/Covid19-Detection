...
def split_wavs(args):
    src_root = args.src_root  # wavfiles
    dst_root = args.dst_root  # clean
    dt = args.delta_time      # 1

    wav_paths = glob('{}/**'.format(src_root), recursive=True)    # paths
    wav_paths = [x for x in wav_paths if '.wav' in x]             # select paths which contain only wavfiles
    dirs = os.listdir(src_root)                                   # directories
    check_dir(dst_root)                                           # check directories in clean folder
    classes = os.listdir(src_root)                                # make classes
    for _cls in classes:
        target_dir = os.path.join(dst_root, _cls)
        check_dir(target_dir)
        src_dir = os.path.join(src_root, _cls)                    # make target directories in dst folder
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fn', type=str, default='breathing-shallow (43)',
